...
export type PronominalAdverbExercise = {
  ...
  guide: { en: string; es: string; de: string }; // guide sentence, matches this exact variant's person
};
```
...
- `PREP_FAMILIES`: 12 entries (one per preposition), each with **12 interchangeable modifier words** (degree/frequency synonyms appropriate to that slot) and a `build()` function producing the blanked Dutch sentence for a given person + question-or-declarative + modifier, plus a `guideBuild(person, adverbId)` function producing the matching `{ en, es, de }` guide sentence conjugated for that *same* person. 6 persons × 12 modifiers = 72 combinations per (adverb, preposition) pair, trimmed to 70.
